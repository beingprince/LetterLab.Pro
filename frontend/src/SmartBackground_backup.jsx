// frontend/src/SmartBackground.jsx
import React, { useMemo } from "react";
import { Box, keyframes, useTheme } from "@mui/material";

/**
 * Sitewide Background (verified)
 * - Sits above all content but below AppBar: zIndex = appBar - 1
 * - Grid always visible (light + dark)
 * - Edge lights clearly visible (stronger)
 * - Light mode only: floating Gmail + AI chips with halos
 * - No backdrop-filter → never blurs your text
 * - pointer-events: none
 */
...
export const SmartBackground = () => {
  ...
  const AURORA_OPACITY = isDark ? 0.65 : 0.5;
  ...
  return (
    <Box
      ...
      sx={{
        position: "fixed",
        inset: 0,
        // >>> key line: put the BG ABOVE everything except AppBar
    ...
    >
      ...
      {/* Edge lights (self-blur → no text blur) */}
      <Box
        sx={{
          position: "absolute",
          inset: 0,
      ...
      {/* Light-mode floating chips + halos */}
